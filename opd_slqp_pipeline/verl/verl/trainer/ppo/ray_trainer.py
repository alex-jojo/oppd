        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    rollout_correction_active = bool(
                        rollout_corr_config
                        and (
                            rollout_corr_config.get("rollout_is", None) is not None
                            or rollout_corr_config.get("rollout_rs", None) is not None
                            or rollout_corr_config.get("rollout_token_veto_threshold", None) is not None
                            or rollout_corr_config.get("bypass_mode", False)
                            or rollout_corr_config.get("use_policy_gradient", False)
                        )
                    )
                    bypass_recomputing_logprobs = bool(
                        rollout_correction_active and rollout_corr_config.get("bypass_mode", False)
                    )
                    slqp_config = self.config.actor_rollout_ref.actor.policy_loss.get("slqp", None)
                    is_slqp_only = bool(
                        slqp_config
                        and slqp_config.get("enabled", False)
                        and slqp_config.get("slqp_only", False)
                    )
                    policy_loss_config = self.config.actor_rollout_ref.actor.policy_loss
                    is_single_minibatch_opd = bool(
                        policy_loss_config.get("only_reverse_kl_advantages", False)
                        and not rollout_correction_active
                        and self.config.actor_rollout_ref.actor.ppo_epochs == 1
                        and self.config.actor_rollout_ref.actor.ppo_mini_batch_size
                        == self.config.data.train_batch_size * self.config.actor_rollout_ref.rollout.n
                    )
                    if is_slqp_only:
                        # Pure SLQP does not consume old policy log-probabilities.
                        # Keep shape-compatible zeros for generic data metrics and
                        # avoid a complete no-grad actor replay.
                        batch.batch["old_log_probs"] = torch.zeros_like(
                            batch.batch["responses"], dtype=torch.float32
                        )
                        metrics["actor/entropy"] = 0.0
                        metrics["monitor/slqp_skipped_old_logprob_replay"] = 1.0
                        metrics["monitor/teacher_present"] = 0.0
                    elif is_single_minibatch_opd:
                        # In this exact on-policy OPD setup, update_policy has one
                        # mini-batch and one PPO epoch, so it uses the differentiable
                        # actor log-prob from that update and its detach as the old
                        # policy. A separate no-grad actor replay here is therefore
                        # unused by both the reverse-KL advantage and policy ratio.
                        batch.batch["old_log_probs"] = torch.zeros_like(
                            batch.batch["responses"], dtype=torch.float32
                        )
                        metrics["actor/entropy"] = 0.0
                        metrics["monitor/opd_skipped_unused_old_logprob_replay"] = 1.0
                    elif bypass_recomputing_logprobs:
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if is_slqp_only:
                            # Pure SLQP does not train on rule-based rewards and
                            # does not consume PPO/GRPO advantages. Keep only
                            # shape-compatible zeros for generic verl monitors.
                            zero_signal = torch.zeros_like(batch.batch["token_level_rewards"])
                            batch.batch["advantages"] = zero_signal
                            batch.batch["returns"] = zero_signal.clone()
                            metrics["monitor/slqp_skipped_advantage_estimation"] = 1.0
                        elif policy_loss_config.get("only_reverse_kl_advantages", False):
                            # OPD constructs its token-level reverse-KL advantage
                            # from current student and teacher log-probabilities
                            # inside update_policy. The generic GRPO advantage is
                            # overwritten there and has no effect on the update.
                            zero_signal = torch.zeros_like(batch.batch["token_level_rewards"])
                            batch.batch["advantages"] = zero_signal
                            batch.batch["returns"] = zero_signal.clone()
                            metrics["monitor/opd_skipped_unused_grpo_advantage"] = 1.0
                        else:
                ...
                # Rule-based math correctness is monitoring only for OPD/SLQP.
                # OPD replaces actor advantages with reverse-KL; SLQP-only replaces
                # the policy loss with its latent objective.
                metrics["monitor/train_accuracy_reward"] = data_metrics["critic/score/mean"]
                # Length-side-effect monitors use only generated, non-special
                # response tokens. They never enter OPD/SLQP training signals.
                generated_mask = batch.batch["response_mask"].bool()
                for token_id in self.tokenizer.all_special_ids:
                    generated_mask = generated_mask & batch.batch["responses"].ne(token_id)
                generated_lengths = generated_mask.sum(dim=1).float()
                sequence_scores = batch.batch["token_level_scores"].sum(dim=1)
                correct_mask = sequence_scores > 0.5
                incorrect_mask = ~correct_mask
                metrics["monitor/generated_response_tokens_mean"] = generated_lengths.mean().item()
                metrics["monitor/correct_response_count"] = correct_mask.sum().item()
                metrics["monitor/incorrect_response_count"] = incorrect_mask.sum().item()
                metrics["monitor/correct_response_tokens_mean"] = (
                    generated_lengths[correct_mask].mean().item() if correct_mask.any() else 0.0
                )
                metrics["monitor/incorrect_response_tokens_mean"] = (
                    generated_lengths[incorrect_mask].mean().item() if incorrect_mask.any() else 0.0
                )
